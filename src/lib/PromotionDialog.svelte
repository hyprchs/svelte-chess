<script lang="ts">
  ...
  const PROMOTION_PIECE_TYPES: PieceType[] = [
    QUEEN,
    ROOK,
    BISHOP,
    KNIGHT,
  ...
  const marginLeft =
    (100 / 8) *
    (orientation === WHITE ? squareFile(square) : 7 - squareFile(square));
...
<div class="dialog {className}">
  {#each PROMOTION_PIECE_TYPES as piece, i}
    {@const putPiecesFromTop =
      (white && orientation === WHITE) || (black && orientation === BLACK)}
